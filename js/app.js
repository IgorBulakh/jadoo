(() => {
    ...
    function initSliders() {
        if (document.querySelector(".testimonials__slider")) new swiper_core_Swiper(".swiper", {
    ...
    let callback = (entries, observer) => {
        entries.forEach((entry => {
            const targetElement = entry.target;
            if (entry.isIntersecting) targetElement.classList.add("show"); else targetElement.classList.remove("show");
        }));
    };
    let observer = new IntersectionObserver(callback, options);
    let someElements = document.querySelectorAll(".cart__item");
    someElements.forEach((someElement => {
        observer.observe(someElement);
    }));
    let someTitleBlock = document.querySelectorAll(".title-block");
    someTitleBlock.forEach((someTitleBlock => {
        observer.observe(someTitleBlock);
    }));
    let someClientBlock = document.querySelectorAll(".clients__item");
    someClientBlock.forEach((someClientBlock => {
        observer.observe(someClientBlock);
    }));
    window["FLS"] = true;
    isWebp();
    menuInit();
